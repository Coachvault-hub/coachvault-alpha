...
function friendlyEngineError(error) {
  ...
  if (/Failed to retrieve the client token|client token/i.test(message)) {
    return 'CoachVault encountered the legacy Vercel client-token upload path. Engine 3.10.4 no longer uses that method for large documents; redeploy this version and retry the file.';
  }
  ...
  if (/SIGNED_PRIVATE_READ_FAILED|signed private read|temporary private read link/i.test(message)) {
    return 'Engine 3.10.4 no longer downloads the large private PDF through the Vercel Function. The model now reads the PDF directly from a short-lived signed private URL.';
  }
  ...
  if (/PRIVATE_BLOB_RETRIEVAL_FAILED|could not retrieve the private file|could not open the private file/i.test(message)) {
    return 'CoachVault stored the document privately, but the Engine could not reopen it for analysis. Engine 3.10.4 uses a signed private read instead of the previous direct read path.';
  }
...
export default function Home() {
  ...
  const [fileJobStatus, setFileJobStatus] = useState('');
  ...
  async function waitForLargePdf(responseId, fileMeta) {
    setFileJobStatus('Analyzing large document…');
    setProgressStep(1);

    for (let attempt = 0; attempt < 120; attempt++) {
      await new Promise(resolve => setTimeout(resolve, 3000));

      const response = await fetch('/api/engine/analyze/file-status', {
        method:'POST',
        headers:{ 'Content-Type':'application/json' },
        body:JSON.stringify({ responseId, fileMeta })
      });

      const raw = await response.text();
      let data = {};
      try { data = raw ? JSON.parse(raw) : {}; }
      catch (_) { throw new Error(raw || `Unexpected large-document status response (${response.status}).`); }

      if (response.status === 202 || data.status === 'processing') {
        setFileJobStatus(data.message || 'Reading PDF text, diagrams, and drill sections…');
        setProgressStep((current) => Math.min(5, Math.max(current, 2)));
        continue;
      }

      if (!response.ok) throw new Error(readableError(data.error) || `Large-document analysis failed (${response.status}).`);

      setFileJobStatus('Document understood. Building the Coach Practice Card…');
      setProgressStep(5);
      return data;
    }

    throw new Error('This document is taking longer than expected to analyze. The upload succeeded, but the analysis job did not finish within the current polling window.');
  }
    ...
    try {
      ...
      try {
        ...
        if (response.status >= 500) {
          throw new Error(`CoachVault received a non-JSON server error (${response.status}). The upload may have succeeded, but the analysis route ended before returning diagnostics. Check the Vercel Function log for /api/engine/analyze; Engine 3.10.4 now labels large-PDF failures by stage.`);
        }
        throw new Error(
          response.status === 413
      ...
      if (response.status === 202 && data.pendingFileJob?.responseId) {
        data = await waitForLargePdf(data.pendingFileJob.responseId, data.pendingFileJob.fileMeta);
      }
      ...
      <header className="globalHeader">
        <Link href="/" className="workspaceBrandHome" aria-label="CoachVault home">
          <div className="brandLockup branded">
            ...
            <small className="engineVersion">Engine 3.10.4</small>
          </div>
        </Link>
        ...
        {active === 'Atlas' && !result && (
          <>
            ...
            <section className={`inputPanel inputPanelTop simplifiedPanel compactAtlasPanel ${loading ? 'engineRunningPanel' : ''}`}>
              ...
              {loading && <EngineProgress step={progressStep} socialJobStatus={socialJobStatus} fileJobStatus={fileJobStatus} />}
...
function EngineProgress({ step, socialJobStatus='', fileJobStatus='' }) {
  const stages = [
  ...
  return <section className="engineAnalysis">
    ...
    <div className="analysisCurrent">
      ...
      <div>
        ...
        <h3>{fileJobStatus || socialJobStatus || stages[current].title}</h3>
        <p>{fileJobStatus ? 'CoachVault will keep checking the document automatically. You do not need to upload it again.' : socialJobStatus ? 'CoachVault will keep checking this video automatically. You do not need to paste the link again.' : stages[current].detail}</p>
      </div>
    </div>
