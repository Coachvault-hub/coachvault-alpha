...
function friendlyEngineError(error) {
  ...
  if (/Failed to retrieve the client token|client token/i.test(message)) {
    return 'CoachVault encountered the legacy Vercel client-token upload path. Engine 3.10.0 no longer uses that method for large documents; redeploy this version and retry the file.';
  }
  ...
  if (/SIGNED_PRIVATE_READ_FAILED|signed private read|temporary private read link/i.test(message)) {
    return 'Engine 3.10.0 no longer downloads the large private PDF through the Vercel Function. The model now reads the PDF directly from a short-lived signed private URL.';
  }
  ...
  if (/PRIVATE_BLOB_RETRIEVAL_FAILED|could not retrieve the private file|could not open the private file/i.test(message)) {
    return 'CoachVault stored the document privately, but the Engine could not reopen it for analysis. Engine 3.10.0 uses a signed private read instead of the previous direct read path.';
  }
    ...
    try {
      ...
      try {
        ...
        if (response.status >= 500) {
          throw new Error(`CoachVault received a non-JSON server error (${response.status}). The upload may have succeeded, but the analysis route ended before returning diagnostics. Check the Vercel Function log for /api/engine/analyze; Engine 3.10.0 now labels large-PDF failures by stage.`);
        }
        throw new Error(
          response.status === 413
      ...
      <header className="globalHeader">
        <Link href="/" className="workspaceBrandHome" aria-label="CoachVault home">
          <div className="brandLockup branded">
            ...
            <small className="engineVersion">Engine 3.10.0</small>
          </div>
        </Link>
