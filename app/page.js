  ...
  return <section className="reviewShell">
    ...
    <div className="reviewGrid">
      <div className="reviewMain">
        ...
        <Field label="Coaching problem"><textarea value={result.coachingProblem || ''} onChange={(e) => updateField('coachingProblem', e.target.value)} /></Field>
        ...
        {(result.coachingConcepts || []).length > 0 && <section className="drillSection"><div className="sectionTitle"><div><small>COACHING CONCEPTS</small><h3>The teachable ideas inside this resource</h3></div></div>{result.coachingConcepts.map((concept, index) => <article className="drillCard" key={`${concept.name}-${index}`}><span>{String(index + 1).padStart(2, '0')}</span><div><h4>{concept.name}</h4><p><b>{concept.category}</b> · {concept.importance}</p><p>{concept.evidence}</p></div></article>)}</section>}
      ...
      <aside className="reviewAside">
        ...
        <div className="coachingPoints"><small>COACHING POINTS</small>{(result.teachingModel?.coachingPoints || result.coachingPoints || []).map((point, index) => <p key={index}>{point}</p>)}</div>
        {(result.teachingModel?.commonMistakes || []).length > 0 && <div className="coachingPoints"><small>COMMON MISTAKES</small>{result.teachingModel.commonMistakes.map((point, index) => <p key={index}>{point}</p>)}</div>}
        {(result.teachingModel?.successIndicators || []).length > 0 && <div className="coachingPoints"><small>SUCCESS INDICATORS</small>{result.teachingModel.successIndicators.map((point, index) => <p key={index}>{point}</p>)}</div>}
        <p><small>ENGINE VERSION</small><br/><b>{result.engineVersion || '1.0.0'}</b></p>
      </aside>
    </div>
